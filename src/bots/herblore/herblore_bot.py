...
from osrs_botlib.input.organic_easing import OrganicEasingConfig
...
from osrs_botlib.anti_detection.attention_drift import AttentionDrift, DriftConfig
from osrs_botlib.safety.emergency_stop import EmergencyStop
from osrs_botlib.safety.login_handler import LoginHandler, LoginConfig
from osrs_botlib.core.base_actions import ActionCategory

from bots.herblore.herblore_states import HerbCleaningStateMachine, BotState
from bots.herblore.herblore_actions import HerbloreActionType
from bots.herblore.herb_inventory import HerbInventoryDetector
from bots.herblore.herb_bank import HerbBankDetector
from bots.herblore.herb_session import HerbSessionTracker, HerbloreSessionStats

# Old imports that still exist in src/
...
class HerbloreBotController:
    ...
    def __init__(self, config_path: Optional[str] = None, overlay_enabled: bool = False):
        ...
        self.inventory = HerbInventoryDetector(
        ...
        from osrs_botlib.safety.base_session import SessionConfig
        self.session = HerbSessionTracker(
    ...
    def _handle_banking_open(self) -> None:
        ...
        time.sleep(self.timing.get_delay(ActionCategory.BANK_OPEN))

        # Verify bank opened
        if self.bank.is_bank_open():
            time.sleep(self.timing.get_post_action_delay(ActionCategory.BANK_OPEN))
    ...
    def _handle_banking_deposit(self) -> None:
        ...
        if use_herb_click:
            ...
            if clean_slots:
                ...
                if completed:
                    time.sleep(self.timing.get_post_action_delay(ActionCategory.BANK_DEPOSIT))
    ...
    def _handle_banking_withdraw(self) -> None:
        ...
        if completed:
            time.sleep(self.timing.get_post_action_delay(ActionCategory.BANK_WITHDRAW))
    ...
    def _handle_banking_close(self) -> None:
        ...
        time.sleep(self.timing.get_post_action_delay(ActionCategory.BANK_CLOSE))
    ...
    def _handle_cleaning(self) -> None:
        ...
        if completed:
            ...
            delay = self.timing.get_delay(ActionCategory.SKILL_ACTION)
